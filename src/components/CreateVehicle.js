...
import { useCreateVehicleMutation } from '../store/apis/vehicleApi';
...
function CreateVehicle() {
    ...
    const [createVehicle, results] = useCreateVehicleMutation();
                ...
                <div>
                    ...
                    <select value={modelName} onChange={(e) => setModelName(e.target.value)}>
                        {models.map((model, index) => (
                            <option key={model.id} value={index+1}>{model.modelName}</option>
                        ))}
                    </select>
                </div>
                <div>
